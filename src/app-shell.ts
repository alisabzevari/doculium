...
export class AppShell extends LitElement {
 ...
 private _router = new Router(this, [
  ...
  { path: '/library/:id', render: () => html`<document-detail></document-detail>` },
 ...
 render() {
  return html`
   <div class="overlay hidden md:hidden ${this.sidebarOpen ? '!block' : ''}" @click=${() => this.sidebarOpen = false}></div>

   <aside class="sidebar bg-base-200 border-r border-base-300 flex flex-col ${this.sidebarOpen ? 'open' : ''}">
    <div class="p-4 border-b border-base-300">
     <h1 class="text-xl font-bold flex items-center gap-2">
       <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" viewBox="0 0 32 32" fill="none">
        ...
        <path d="M23 12h-5a1.5 1.5 0 0 1-1.5-1.5V6" fill="#e5e7eb" opacity="0.5"/>
        <path d="M23 12h-5a1.5 1.5 0 0 1-1.5-1.5V6l6.5 6Z" fill="#d1d5db" opacity="0.3"/>
    ...
    <nav class="flex-1 p-2 space-y-1">
     <button class="w-full btn btn-ghost justify-start gap-3 ${this._isActive('/') ? 'btn-active' : ''}" @click=${() => this._navigate('/')}>
     ...
     <button class="w-full btn btn-ghost justify-start gap-3 ${this._isActive('/scan') ? 'btn-active' : ''}" @click=${() => this._navigate('/scan')}>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
     ...
     <button class="w-full btn btn-ghost justify-start gap-3 ${this._isActive('/library') ? 'btn-active' : ''}" @click=${() => this._navigate('/library')}>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
     ...
     <button class="w-full btn btn-ghost justify-start gap-3 ${this._isActive('/settings') ? 'btn-active' : ''}" @click=${() => this._navigate('/settings')}>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="p-4 border-t border-base-300 text-xs opacity-50">
     ${this.urgentCount > 0 ? html`<span class="badge badge-error badge-xs">${this.urgentCount} urgent</span>` : ''}
